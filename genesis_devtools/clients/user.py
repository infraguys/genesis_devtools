...
def list_users(
    ...
    return client.filter(c.USER_COLLECTION, **filters)
...
def add_user(
    client: http_client.CollectionBaseClient,
    user: dict[str, tp.Any],
) -> dict[str, tp.Any]:
    try:
        user_resp = client.create(c.USER_COLLECTION, data=user)
...
def delete_user(
    ...
    try:
        ...
        raise click.ClickException(f"user with UUID {uuid} not found")
