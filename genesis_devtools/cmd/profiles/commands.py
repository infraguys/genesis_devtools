...
from genesis_devtools.clients import profile as profile_lib
...
def list_profiles(ctx: click.Context) -> None:
    ...
    profiles = profile_lib.list_profiles(client)
...
def show_profile_cmd(
    ...
    if not utils.is_valid_uuid(uuid):
        profiles = profile_lib.list_profiles(client, name=uuid)
        if profiles:
            uuid = profiles[0]["uuid"]
        else:
            raise click.ClickException(f"Profile with name {uuid} not found")
    profile = profile_lib.get_profile(client, uuid)
...
def delete_profile_cmd(
    ...
    if not utils.is_valid_uuid(uuid):
        ...
        if profiles:
            ...
            raise click.ClickException(f"Profile with name {uuid} not found")
    profile_lib.delete_profile(client, uuid)
...
def activate_profile_cmd(
    ...
    if not utils.is_valid_uuid(uuid):
        ...
        if profiles:
            ...
            raise click.ClickException(f"Profile with name {uuid} not found")
    profile_lib.activate_profile(client, uuid)
...
def add_profile_cmd(
    ...
    if uuid is None:
        uuid = sys_uuid.uuid4()
    profile = profile_lib.add_profile(
        client,
        {
            "uuid": str(uuid),
