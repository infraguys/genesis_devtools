...
def _bootstrap_core(
    ...
    try:
        ...
        if save_admin_password_file:
            if os.path.exists(save_admin_password_file):
                logger.warn(
                    f"Admin password file {save_admin_password_file} already exists. "
                    "Skipping saving the password."
                )
            else:
                os.makedirs(os.path.dirname(save_admin_password_file), exist_ok=True)
                with open(save_admin_password_file, "w") as f:
